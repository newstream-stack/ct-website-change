    ...
    <div className="pt-[190px] md:pt-48 pb-32 px-5 md:px-12 lg:px-20 min-h-screen bg-theme-bg text-theme-text transition-colors duration-500">
      <div className="max-w-6xl mx-auto">
        <div className="mb-12 md:mb-16 border-b border-theme-text/15 pb-7 md:pb-9">
          <p className="font-display text-xs tracking-[0.3em] text-brand-red uppercase mb-4">Articles by</p>
          <h1 className="font-serif text-4xl md:text-6xl font-black tracking-widest leading-tight">{author}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
          {articles.map((article) => (
            <button key={article.id} onClick={() => openArticle(article.id)} className="group text-left">
              ...
              <p className="mt-5 pt-4 border-t border-theme-text/10 font-display text-[10px] tracking-[0.16em] text-theme-text/45">{article.date}</p>
            </button>
          ))}
        </div>
      </div>
