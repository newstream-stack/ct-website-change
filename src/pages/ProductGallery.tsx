  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start gap-4 border-t border-theme-text/20 pt-4 transition-colors">
        ...
        <span className="font-display font-bold text-xl text-brand-red whitespace-nowrap">
...
function GalleryAd({ onGetTickets }: { onGetTickets: () => void }) {
  return (
    <div className="gallery-item w-[85vw] md:w-[400px] flex-shrink-0 group cursor-pointer border border-theme-text/20 bg-theme-text/5 relative overflow-hidden flex flex-col items-center justify-center p-8 text-center hover:border-brand-red/50 transition-colors aspect-[3/4] mb-12 mt-6">
      <img src="https://images.unsplash.com/photo-1499209974431-9dddcece7f88?auto=format&fit=crop&q=80&w=800" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition-all duration-700" alt="Art Fair 2026" loading="lazy" decoding="async" />
      <div className="absolute inset-0 bg-gradient-to-b from-theme-bg/80 to-theme-bg/95 transition-colors" />
      ...
      <button type="button" onClick={onGetTickets} className="relative z-10 font-display font-bold uppercase tracking-widest text-[10px] border border-theme-text/30 px-6 py-3 text-theme-text hover:bg-theme-text hover:text-theme-bg transition">Get Tickets</button>
    </div>
  );
}

export default function ProductGallery({ onSelectProduct, goToCategory }: ProductGalleryProps) {
  ...
  return (
    <div className="pt-[190px] md:pt-32 pb-24 min-h-screen flex flex-col bg-theme-bg text-theme-text transition-colors duration-500">
      ...
      <div ref={galleryRef} className="gallery-track flex gap-6 md:gap-12 px-6 md:px-12 lg:px-20 pb-20 flex-grow items-center">
        {products[0] && <ProductCard product={products[0]} index={0} onSelect={onSelectProduct} />}
        <GalleryAd onGetTickets={() => goToCategory('活動報名')} />
        {products.slice(1).map((product, index) => (
          <ProductCard key={product.id} product={product} index={index + 1} onSelect={onSelectProduct} />
        ))}
