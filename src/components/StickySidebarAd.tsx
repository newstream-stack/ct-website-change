...
export default function StickySidebarAd({ ad }: Props) {
  return (
    <a href={getSafeExternalUrl(ad.link)} target="_blank" rel="noopener noreferrer" className="sticky top-32 border border-theme-text/10 bg-theme-text/5 aspect-[3/4] sm:aspect-auto sm:h-[400px] lg:aspect-[3/4] flex flex-col items-center justify-center p-6 md:p-8 text-center relative group cursor-pointer overflow-hidden rounded-sm transition-colors block">
        <img src={ad.imageUrl} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 group-hover:opacity-80 transition-all duration-700" alt={ad.title} />
